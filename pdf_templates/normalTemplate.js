...
const fs = require("fs")
const { generateInvoiceQRCode } = require("../utils/qrCodeGenerator")
	...
	return new Promise(async (resolve, reject) => {
		try {
			...
			doc
				.fontSize(10)
				.font("Helvetica-Bold")
				.text("Tanggal:", metaX, metaY + 30)
			doc
				.font("Helvetica")
				.text(formatTanggal(invoice.invoice_date), metaX, metaY + 42)
			...
			if (invoice.due_date) {
				doc
					...
					.fillColor("black")
					.font("Helvetica-Bold")
					.text("Jatuh Tempo:", 400, contentY)
					.font("Helvetica")
					.text(formatTanggal(invoice.due_date), 400, contentY + 15)
			}
			...
			;(invoice.items || []).forEach((item, i) => {
				...
				const descHeight = doc.heightOfString(itemDesc, {
				...
				doc.text(item.quantity || item.qty || 0, col.qty, y + 6, {
					width: 40,
					align: "center",
				})
				doc.text(item.unit || "-", col.unit, y + 6, {
					width: 50,
					align: "center",
				})
				doc.text(
					formatRupiah(item.price || item.unit_price),
					col.price,
					y + 6,
				...
				doc.text(formatRupiah(lineTotal), col.total, y + 6, {
					width: 65,
					align: "right",
				})
			...
			const drawSummary = (label, value, isBold = false) => {
				...
				doc.text(formatRupiah(value), summaryXValue, y, {
					width: 95,
					align: "right",
				})
				y += 16
			...
			;(invoice.taxes || []).forEach((tax) => {
				const taxAmount = Math.round(
					tax.amount || (subtotal * (tax.percentage || 0)) / 100,
				)
			...
			try {
				// Generate QR code dengan share token
				// Construct base URL dari environment atau default
				const baseUrl =
					process.env.BASE_URL || `http://localhost:${process.env.PORT || 3000}`

				console.log("🔍 PDF Generator Debug:")
				console.log("  - invoice.share_token:", invoice.share_token)
			...
			doc
				.fontSize(10)
				.font("Helvetica-Bold")
				.text(invoice.company_name, 50, y + 100)
